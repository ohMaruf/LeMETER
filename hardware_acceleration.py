...
def print_device_memory_snapshot(device: torch.device, step_name: str):
    if device.type == "cuda":
        ...
        logger.info(
            f"[{step_name}] Allocated VRAM: {allocated:.2f}GB | Reserved VRAM: {reserved:.2f}GB"
        )  # noqa: E501
    elif device.type == "mps":
        allocated = torch.mps.current_allocated_memory() / (1024**3)
        driver_mem = torch.mps.driver_allocated_memory() / (1024**3)
        logger.info(
            f"[{step_name}] Tensors VRAM: {allocated:.2f}GB | Metal Driver Total VRAM: {driver_mem:.2f}GB"
        )  # noqa: E501
...
def _rx9060xt_config(config: Config) -> torch.device:
    ...
    _set_environ("TORCH_BLAS_PREFER_HIPBLASLT", "0")
